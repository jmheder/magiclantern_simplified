...
static CONFIG_INT("raw.use.srm.memory", use_srm_memory, 0);
...
static void FAST process_frame()
{
    ...
    //raw_lv_redirect_edmac_adv(ptr,res_x*14/8, res_y); // extract w,x image from RAW EDMAC, centered correctly within the active image 
    raw_lv_redirect_edmac(ptr); // extract w,x image from RAW EDMAC, centered correctly within the active image 
...
extern int start_stop_RAW_recording;
...
#if 0 // direct buffering .. no can do at this momment .. 
